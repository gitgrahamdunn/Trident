...
Docs: https://docs.trident.ai

## Unreleased

### Release

- First Trident fork release surface: separate `trident` package, CLI entrypoint, state root, and migration path from `~/.openclaw`.
- Public package metadata now publishes as `trident` with a Trident-specific README and release-facing install guidance.
- Release branding cleanup continues to remove user-facing OpenClaw naming from CLI help, onboarding, daemon, doctor, and status surfaces while keeping compatibility-critical internals unchanged.
- `docs.trident.ai` references are placeholders for this release unless a live docs site is available.
