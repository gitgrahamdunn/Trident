...
const SEARCH_TOOL = "https://docs.trident.ai/mcp.SearchTrident";
...
async function docsSearchCommand(queryParts, runtime) {
	...
	if (!query) {
		const docs = formatDocsLink("/", "docs.trident.ai");
		if (isRich()) {
			runtime.log(`${theme.muted("Docs:")} ${docs}`);
				runtime.log(`${theme.muted("Search:")} ${formatCliCommand("trident docs \"your query\"")}`);
		} else {
				runtime.log("Docs: https://docs.trident.ai/");
				runtime.log(`Search: ${formatCliCommand("trident docs \"your query\"")}`);
		}
		return;
	}
...
function registerDocsCli(program) {
	program.command("docs").description("Search the live Trident docs").argument("[query...]", "Search query").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/docs", "docs.trident.ai/cli/docs")}\n`).action(async (queryParts) => {
