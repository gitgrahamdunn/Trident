...
function resolveRawHomeDir(env, homedir) {
	const explicitHome = normalize(env.TRIDENT_HOME ?? env.OPENCLAW_HOME);
	if (explicitHome) {
...
const NEW_STATE_DIRNAME = ".trident";
const CONFIG_FILENAME = "trident.json";
...
function resolveStateDir(env = process.env, homedir = envHomedir(env)) {
	...
	const override = env.TRIDENT_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
...
function resolveCanonicalConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env))) {
	const override = env.TRIDENT_CONFIG_PATH?.trim() || env.OPENCLAW_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
...
function resolveConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env)), homedir = envHomedir(env)) {
	const override = env.TRIDENT_CONFIG_PATH?.trim() || env.OPENCLAW_CONFIG_PATH?.trim();
	...
	const stateOverride = env.TRIDENT_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim();
...
function resolveDefaultConfigCandidates(env = process.env, homedir = envHomedir(env)) {
	const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
	const explicit = env.TRIDENT_CONFIG_PATH?.trim() || env.OPENCLAW_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
	if (explicit) return [resolveUserPath(explicit, env, effectiveHomedir)];
	const candidates = [];
	const openclawStateDir = env.TRIDENT_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
	if (openclawStateDir) {
...
function resolveOAuthDir(env = process.env, stateDir = resolveStateDir(env, envHomedir(env))) {
	const override = env.TRIDENT_OAUTH_DIR?.trim() || env.OPENCLAW_OAUTH_DIR?.trim();
	if (override) return resolveUserPath(override, env, envHomedir(env));
...
//#endregion
export { expandHomePrefix as _, resolveCanonicalConfigPath as a, resolveRequiredHomeDir as b, resolveDefaultConfigCandidates as c, resolveIsNixMode as d, resolveLegacyStateDirs as f, resolveStateDir as g, resolveOAuthPath as h, isNixMode as i, resolveGatewayLockDir as l, resolveOAuthDir as m, DEFAULT_GATEWAY_PORT as n, resolveConfigPath as o, resolveNewStateDir as p, STATE_DIR as r, resolveConfigPathCandidate as s, CONFIG_PATH as t, resolveGatewayPort as u, resolveEffectiveHomeDir as v, resolveHomeRelativePath as y };
