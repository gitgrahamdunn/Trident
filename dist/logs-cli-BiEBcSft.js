...
import { M as formatLocalIsoWithOffset, N as isValidTimeZone, h as clearActiveProgressLine } from "./subsystem-D2xHvZZd.js";
import { n as isRich, r as theme, t as colorize } from "./theme-CipOb_We.js";
import "./utils-DwZbjiC4.js";
import { Es as buildGatewayConnectionDetails } from "./reply-Bm8VrLQh.js";
...
function createLogWriters() {
	const writer = createSafeStreamWriter({
		...
		onBrokenPipe: (err, stream) => {
			...
			const message = `trident logs: output ${stream === process.stdout ? "stdout" : "stderr"} closed (${code}). Stopping tail.`;
			try {
...
function emitGatewayError(err, opts, mode, rich, emitJsonLine, errorLine) {
	...
	const hint = `Hint: run \`${formatCliCommand("trident doctor")}\`.`;
