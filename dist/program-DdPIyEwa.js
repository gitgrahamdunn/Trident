import { F as ALLOWED_LOG_LEVELS, J as hasHelpOrVersion, K as getVerboseFlag, L as tryParseLogLevel, V as getCommandPathWithRootOptions, Y as hasRootVersionAlias, p as defaultRuntime, q as hasFlag, w as setVerbose } from "./subsystem-BDbeCphF.js";
...
import { c as escapeRegExp } from "./utils-B88a096J.js";
...
import { n as resolveCommitHash } from "./git-commit-B0vIx0co.js";
import "./prompt-style-C94HXvSH.js";
import { t as formatDocsLink } from "./links-BuWtQKK5.js";
import "./cli-utils-CCDylRkX.js";
import { n as resolveCliChannelOptions } from "./channel-options-CAEyWh-A.js";
import { t as getSubCliCommandsWithSubcommands } from "./register.subclis-DAJgItwI.js";
import { a as registerProgramCommands, r as getCoreCliCommandsWithSubcommands } from "./command-registry-nxBYxAWJ.js";
import { r as setProgramContext } from "./program-context-CeOnIsy3.js";
import "./ports-CccLtXdM.js";
import { n as formatCliBannerLine, r as hasEmittedCliBanner, t as emitCliBanner } from "./banner-BsdN6v3E.js";
...
const EXAMPLES = [
	["trident models --help", "Show detailed help for the models command."],
	["trident channels login --verbose", "Link personal WhatsApp Web and show QR + connection logs."],
	["trident message send --target +15555550123 --message \"Hi\" --json", "Send via your web session and print JSON result."],
	["trident gateway --port 18789", "Run the WebSocket Gateway locally."],
	["trident --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
	["trident gateway --force", "Kill anything bound to the default gateway port, then start it."],
	["trident gateway ...", "Gateway control via WebSocket."],
	["trident agent --to +15555550123 --message \"Run summary\" --deliver", "Talk directly to the agent using the Gateway; optionally send the reply."],
	["trident message send --channel telegram --target @mychat --message \"Hi\"", "Send via your Telegram bot."]
];
function configureProgramHelp(program, ctx) {
	program.name(CLI_NAME).description("").version(ctx.programVersion).option("--dev", "Dev profile: isolate state under ~/.trident-dev, default gateway port 19001, and shift derived ports (browser/canvas)").option("--profile <name>", "Use a named profile (isolates TRIDENT_STATE_DIR/TRIDENT_CONFIG_PATH under ~/.trident-<name>)").option("--log-level <level>", `Global log level override for file + console (${CLI_LOG_LEVEL_VALUES})`, parseCliLogLevelOption);
	...
	if (hasFlag(process.argv, "-V") || hasFlag(process.argv, "--version") || hasRootVersionAlias(process.argv)) {
		...
		console.log(commit ? `Trident ${ctx.programVersion} (${commit})` : `Trident ${ctx.programVersion}`);
		process.exit(0);
	}
	...
	program.addHelpText("afterAll", ({ command }) => {
		...
		const docs = formatDocsLink("/cli", "github.com/gitgrahamdunn/trident-cli/tree/main/docs");
...
function loadConfigGuardModule() {
	configGuardModulePromise ??= import("./config-guard-BN004cqI.js").then((n) => n.t);
	return configGuardModulePromise;
}
function loadPluginRegistryModule() {
	pluginRegistryModulePromise ??= import("./plugin-registry-B0_Af19D.js").then((n) => n.n);
