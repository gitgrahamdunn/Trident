...
import { G as formatHelpExamples, Op as formatSessionArchiveTimestamp, Pg as readConfigFileSnapshot } from "./auth-profiles-DRjqKE3G.js";
import "./agent-scope-CZIF93u7.js";
import { g as resolveHomeDir, m as pathExists, v as resolveUserPath, x as shortenHomePath } from "./utils-B88a096J.js";
...
import { n as resolveRuntimeServiceVersion } from "./version-DcA9ITyc.js";
...
import { n as isPathWithin, t as buildCleanupPlan } from "./cleanup-utils-BMm3OQGr.js";
...
function buildBackupArchiveRoot(nowMs = Date.now()) {
	return `${formatSessionArchiveTimestamp(nowMs)}-trident-backup`;
}
...
async function resolveBackupPlanFromDisk(params = {}) {
	...
	if (includeWorkspace && configSnapshot.exists && !configSnapshot.valid) throw new Error(`Config invalid at ${shortenHomePath(configSnapshot.path)}. Trident cannot reliably discover custom workspaces for backup. Fix the config or rerun with --no-include-workspace for a partial backup.`);
...
async function createBackupArchive(opts = {}) {
	...
	if (plan.included.length === 0) throw new Error(onlyConfig ? "No Trident config file was found to back up." : "No local Trident state was found to back up.");
	...
	const tempDir = await fs$1.mkdtemp(path.join(os.tmpdir(), "trident-backup-"));
...
function registerBackupCommand(program) {
	const backup = program.command("backup").description("Create and verify local backup archives for Trident state").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.trident.ai/cli/backup")}\n`);
	backup.command("create").description("Write a backup archive for config, credentials, sessions, and workspaces").option("--output <path>", "Archive path or destination directory").option("--json", "Output JSON", false).option("--dry-run", "Print the backup plan without writing the archive", false).option("--verify", "Verify the archive after writing it", false).option("--only-config", "Back up only the active JSON config file", false).option("--no-include-workspace", "Exclude workspace directories from the backup").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["trident backup create", "Create a timestamped backup in the current directory."],
		["trident backup create --output ~/Backups", "Write the archive into an existing backup directory."],
		["trident backup create --dry-run --json", "Preview the archive plan without writing any files."],
		["trident backup create --verify", "Create the archive and immediately validate its manifest and payload layout."],
		["trident backup create --no-include-workspace", "Back up state/config without agent workspace files."],
		["trident backup create --only-config", "Back up only the active JSON config file."]
	])}`).action(async (opts) => {
	...
	backup.command("verify <archive>").description("Validate a backup archive and its embedded manifest").option("--json", "Output JSON", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([["trident backup verify ./2026-03-09T00-00-00.000Z-trident-backup.tar.gz", "Check that the archive structure and manifest are intact."], ["trident backup verify ~/Backups/latest.tar.gz --json", "Emit machine-readable verification output."]])}`).action(async (archive, opts) => {
		await runCommandWithRuntime(defaultRuntime, async () => {
