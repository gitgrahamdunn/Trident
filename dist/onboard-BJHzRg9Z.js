import { m as restoreTerminalState, p as defaultRuntime } from "./subsystem-BDbeCphF.js";
import { u as resolveGatewayPort } from "./paths-dQ_clcF4.js";
import { Dt as randomToken, Mt as waitForGatewayReachable, Ot as resolveControlUiLinks, Pg as readConfigFileSnapshot, Rg as writeConfigFile, St as normalizeGatewayTokenInput, _t as ensureWorkspaceAndSessions, bt as handleReset, mt as applyWizardMetadata, pt as DEFAULT_WORKSPACE } from "./auth-profiles-DRjqKE3G.js";
import { t as formatCliCommand } from "./command-format-BTnLVWI8.js";
import { v as resolveUserPath } from "./utils-B88a096J.js";
import { s as isValidEnvSecretRefId } from "./types.secrets-BvNoSfUf.js";
import { jt as resolveDefaultSecretProviderAlias } from "./plugins-C-z6VZlO.js";
...
import "./daemon-runtime-Cefje0FR.js";
import { t as runOnboardingWizard } from "./onboarding-E9frfTZz.js";
import { n as logConfigUpdated } from "./logging-DRp8gftI.js";
import { i as normalizeLegacyOnboardAuthChoice, r as isDeprecatedAuthChoice, t as ONBOARD_PROVIDER_AUTH_FLAGS } from "./onboard-provider-auth-flags-BcMu_ogf.js";
import { t as applyOnboardingLocalWorkspaceConfig } from "./onboard-config-DLOPfX2Z.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-B6O2Y9L1.js").then((n) => n.r);
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-BHRgvw1Q.js").then((n) => n.t);
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-DeCrgU3G.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-BC_AS4s6.js");
		...
		if (!daemonInstall.installed && !opts.skipHealth) {
			logNonInteractiveOnboardingFailure({
				...
				hints: daemonInstall.skippedReason === "systemd-user-unavailable" ? ["Fix: rerun without `--install-daemon` for one-shot setup, or enable a working user-systemd session and retry.", "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `trident gateway run` or `trident agent --local`."] : [`Run \`${formatCliCommand("trident gateway status --deep")}\` for more detail.`]
			});
			runtime.exit(1);
			return;
		}
		...
		if (!probe.ok) {
			...
			logNonInteractiveOnboardingFailure({
				...
				hints: !opts.installDaemon ? [
					...
					`Fix: start \`${formatCliCommand("trident gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
				...
				].filter((value) => Boolean(value)) : [`Run \`${formatCliCommand("trident gateway status --deep")}\` for more detail.`]
			});
			runtime.exit(1);
			return;
		}
...
async function runNonInteractiveOnboarding(opts, runtime = defaultRuntime) {
	...
	if (snapshot.exists && !snapshot.valid) {
		runtime.error(`Config invalid. Run \`${formatCliCommand("trident doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
...
async function onboardCommand(opts, runtime = defaultRuntime) {
	...
	if (process.platform === "win32") runtime.log([
		"Windows detected - Trident runs great on WSL2!",
		"Native Windows might be trickier.",
		"Quick setup: wsl --install (one command, one reboot)",
		"Guide: https://docs.trident.ai/windows"
	].join("\n"));
