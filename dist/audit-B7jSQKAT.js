...
import { Ag as createConfigIO, Fu as listInterpreterLikeSafeBins, Gp as execDockerRaw, Gs as buildGatewayConnectionDetails, Kp as getBlockedBindReason, Lp as normalizeMessageChannel, Lu as resolveMergedSafeBinProfileFixtures, Sm as resolveProfile, Td as resolveNativeSkillsEnabled, Ug as isDangerousNetworkMode, Vm as resolveBrowserControlAuth, Wg as normalizeNetworkMode, Yt as isDangerousNameMatchingEnabled, ag as resolveToolProfilePolicy, ai as pickSandboxToolPolicy, ch as inspectPathPermissions, ct as readJsonBodyWithLimit, eg as resolveSandboxConfigForAgent, hr as hasResolvedCredentialValue, lh as safeStat, lr as inspectReadOnlyChannelAccount, lt as requestBodyErrorToText, mr as hasConfiguredUnavailableCredentialStatus, ng as resolveSandboxToolPolicyForAgent, nn as resolveDmAllowState, oh as formatPermissionDetail, or as listAgentWorkspaceDirs, pg as SANDBOX_BROWSER_SECURITY_HASH_EPOCH, r_ as normalizeTrustedSafeBinDirs, sh as formatPermissionRemediation, ti as isToolAllowedByPolicies, va as readChannelAllowFromStore, vh as resolveGatewayAuth, w_ as INCLUDE_KEY, wd as resolveNativeCommandsEnabled, xm as resolveBrowserConfig } from "./auth-profiles-DRjqKE3G.js";
import { t as formatCliCommand } from "./command-format-BTnLVWI8.js";
import { N as resolveAgentModelFallbackValues, P as resolveAgentModelPrimaryValue, f as resolveDefaultAgentId, r as listAgentIds } from "./agent-scope-CZIF93u7.js";
import { E as parseAgentSessionKey, c as normalizeAgentId } from "./session-key-51LnISpq.js";
...
import { E as normalizePluginsConfig, i as loadWorkspaceSkillEntries } from "./skills-CtzUimzY.js";
import { m as MANIFEST_KEY } from "./frontmatter-D0K3qXQH.js";
import { n as isPathInside, t as extensionUsesSkippedScannerPath } from "./scan-paths-BubMcdO4.js";
import { n as listChannelPlugins, ut as resolveChannelDefaultAccountId } from "./plugins-C-z6VZlO.js";
import { n as DEFAULT_GATEWAY_HTTP_TOOL_DENY } from "./dangerous-tools-BQrJ_PWn.js";
import { t as scanDirectoryWithSummary } from "./skill-scanner-Ms0uZYdw.js";
import { r as probeGateway, t as resolveGatewayProbeAuthSafe } from "./probe-auth-B1lyIY0x.js";
import { r as resolveNodeCommandAllowlist, t as DEFAULT_DANGEROUS_NODE_COMMANDS } from "./node-command-policy-Byx2jedy.js";
import { c as isNumericTelegramUserId, l as normalizeTelegramAllowFromEntry, s as isZalouserMutableGroupEntry, t as isDiscordMutableAllowEntry } from "./mutable-allowlist-detectors-B5fmawYD.js";
import { t as inferParamBFromIdOrName } from "./model-param-b-hCbQ8gxK.js";
...
async function collectChannelSecurityFindings(params) {
	...
	const warnDmPolicy = async (input) => {
		...
		if (dmScope === "main" && isMultiUserDm) findings.push({
			...
			remediation: "Run: " + formatCliCommand("trident config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate DM sessions per sender."
		});
	};
		...
		for (const accountId of orderedAccountIds) {
			...
			if (plugin.id === "discord") {
				...
				addDiscordNameBasedEntries({
					...
					source: "~/.trident/credentials/discord-allowFrom.json"
				});
...
function collectSyncedFolderFindings(params) {
	...
	if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) findings.push({
		...
		remediation: `Keep OPENCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("trident security audit --fix")}".`
	});
	return findings;
}
...
async function collectSandboxBrowserHashLabelFindings(params) {
	...
	if (missingHash.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("trident sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	...
	if (nonLoopbackPublished.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("trident sandbox recreate --browser --all")} (add --force to skip prompt), then verify published ports are bound to 127.0.0.1.`
	});
	return findings;
}
async function collectPluginsTrustFindings(params) {
	...
	if (npmPluginInstalls.length > 0) {
		...
		if (pluginVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `trident plugins update --all` (or reinstall affected plugins) to refresh install metadata."
		});
	}
	...
	if (npmHookInstalls.length > 0) {
		...
		if (hookVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `trident hooks update --all` (or reinstall affected hooks) to refresh install metadata."
		});
	}
	return findings;
}
	...
	const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
		...
		onReadError: (err) => {
			findings.push({
				...
				remediation: "Check file permissions and plugin layout, then rerun `trident security audit --deep`."
			});
		}
	});
	for (const pluginName of pluginDirs) {
		...
		if (summary.critical > 0) {
			...
			findings.push({
				...
				remediation: "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Trident extensions state directory."
			});
		...
		for (const entry of entries) {
			...
			const summary = await getCodeSafetySummary({
				...
				findings.push({
					...
					remediation: "Check file permissions and skill layout, then rerun `trident security audit --deep`."
				});
				return null;
			});
			if (!summary) continue;
...
async function collectFilesystemFindings(params) {
	...
	if (stateDirPerms.ok) {
		...
		if (stateDirPerms.worldWritable) findings.push({
			...
			detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Trident state.`,
			remediation: formatPermissionRemediation({
			...
			detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Trident state.`,
			remediation: formatPermissionRemediation({
				targetPath: params.stateDir,
				perms: stateDirPerms,
				isDir: true,
...
function collectBrowserControlFindings(cfg, env) {
	...
	try {
		...
		findings.push({
			...
			detail: String(err),
			remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("trident security audit --deep")}".`
		});
		return findings;
	}
...
async function runSecurityAudit(opts) {
	...
	if (deep?.gateway?.attempted && !deep.gateway.ok) findings.push({
		...
		remediation: `Run "${formatCliCommand("trident status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("trident security audit --deep")}".`
	});
	if (deepProbeResult?.authWarning) findings.push({
		...
		remediation: `Set OPENCLAW_GATEWAY_TOKEN/OPENCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("trident security audit --deep")}".`
	});
