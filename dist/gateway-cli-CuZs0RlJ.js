...
async function readDescendantSubagentFallbackReply(params) {
	if (process.env.OPENCLAW_ALLOW_DESCENDANT_SUMMARY_FALLBACK !== "1") return;
...
async function dispatchCronDelivery(params) {
	...
	const finalizeTextDelivery = async (delivery) => {
		...
		const hadDescendants = activeSubagentRuns > 0 || expectedSubagentFollowup || Boolean(completedDescendantReply);
		...
		if (hadDescendants && synthesizedText.trim() === initialSynthesizedText && isLikelyInterimCronMessage(initialSynthesizedText) && initialSynthesizedText.toUpperCase() !== "NO_REPLY".toUpperCase()) {
			return params.withRunSession({
				status: "error",
				error: "Descendant orchestration finished without a grounded final reply; refusing to synthesize completion from child summaries.",
				summary,
				outputText: initialSynthesizedText,
				deliveryAttempted,
				...params.telemetry
			});
		}
...
async function applySessionsPatchToStore(params) {
	...
	const effectiveSessionDefault = (() => {
		if (subagentModelHint) {
			const parsed = parseModelRef(subagentModelHint, resolvedDefault.provider);
	...
	if ("model" in patch) {
		...
		if (raw === null) applyModelOverrideToSessionEntry({
			...
			const isDefault = resolved.ref.provider === effectiveSessionDefault.provider && resolved.ref.model === effectiveSessionDefault.model;
