import { m as restoreTerminalState } from "./subsystem-BDbeCphF.js";
import "./paths-dQ_clcF4.js";
import "./theme-H80Q3Qtv.js";
import "./boolean-DTgd5CzD.js";
import { Et as probeGatewayReachable, Mt as waitForGatewayReachable, Ot as resolveControlUiLinks, gt as detectBrowserOpenSupport, vt as formatControlUiSshHint, wt as openUrl } from "./auth-profiles-DRjqKE3G.js";
import { r as resolveCliName, t as formatCliCommand } from "./command-format-BTnLVWI8.js";
import { y as DEFAULT_BOOTSTRAP_FILENAME } from "./agent-scope-CZIF93u7.js";
import { m as pathExists, v as resolveUserPath } from "./utils-B88a096J.js";
...
import { i as isSystemdUserServiceAvailable } from "./systemd---Z1c197.js";
import { r as healthCommand } from "./health-DSTtGBUV.js";
import { t as ensureControlUiAssetsBuilt } from "./control-ui-assets-2PDMZv23.js";
import { t as resolveOnboardingSecretInputString } from "./onboarding.secret-input-BhEDjlfg.js";
...
import { t as runTui } from "./tui-D5NPGjKV.js";
...
async function finalizeOnboardingWizard(options) {
	...
	if (process.platform === "linux" && systemdAvailable) {
		const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger-CdgSjHC6.js").then((n) => n.r);
	...
	if (!opts.skipUi && gatewayProbe.ok) {
		...
		await prompter.note([
			...
			"Stored in: ~/.trident/trident.json (gateway.auth.token) or OPENCLAW_GATEWAY_TOKEN.",
			`View token: ${formatCliCommand("trident config get gateway.auth.token")}`,
			`Generate token: ${formatCliCommand("trident doctor --generate-gateway-token")}`,
			"Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
			`Open the dashboard anytime: ${formatCliCommand("trident dashboard --no-open")}`,
		...
		if (hatchChoice === "tui") {
			...
			await prompter.note([
				...
				controlUiOpened ? "Opened in your browser. Keep that tab to control Trident." : "Copy/paste this URL in a browser on this machine to control Trident.",
				controlUiOpenHint
			].filter(Boolean).join("\n"), "Dashboard ready");
		} else await prompter.note(`When you're ready: ${formatCliCommand("trident dashboard --no-open")}`, "Later");
	} else if (opts.skipUi) await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
	await prompter.note(["Back up your agent workspace.", "Docs: https://docs.trident.ai/concepts/agent-workspace"].join("\n"), "Workspace backup");
	await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.trident.ai/security", "Security");
	...
	if (webSearchProvider) {
		const { SEARCH_PROVIDER_OPTIONS, resolveExistingKey, hasExistingKey, hasKeyInEnv } = await import("./onboard-search-CeLQ2E2N.js");
		...
		if (webSearchEnabled !== false && hasKey) await prompter.note([
			...
			`  ${formatCliCommand("trident configure --section web")}`,
			"",
			`Get your key at: ${entry?.signupUrl ?? "https://docs.trident.ai/tools/web"}`,
			"Docs: https://docs.trident.ai/tools/web"
		].join("\n"), "Web search");
		else await prompter.note([
			`Web search (${label}) is configured but disabled.`,
			`Re-enable: ${formatCliCommand("trident configure --section web")}`,
			"",
			"Docs: https://docs.trident.ai/tools/web"
		].join("\n"), "Web search");
	} else {
		const { SEARCH_PROVIDER_OPTIONS, hasExistingKey, hasKeyInEnv } = await import("./onboard-search-CeLQ2E2N.js");
		const legacyDetected = SEARCH_PROVIDER_OPTIONS.find((e) => hasExistingKey(nextConfig, e.value) || hasKeyInEnv(e));
		if (legacyDetected) await prompter.note([`Web search is available via ${legacyDetected.label} (auto-detected).`, "Docs: https://docs.trident.ai/tools/web"].join("\n"), "Web search");
		else await prompter.note([
	...
	await prompter.note("What now: https://trident.ai/showcase (\"What People Are Building\").", "What now");
	await prompter.outro(controlUiOpened ? "Onboarding complete. Dashboard opened; keep that tab to control Trident." : "Onboarding complete. Use the dashboard link above to control Trident.");
	return { launchedTui };
}
//#endregion
