...
import { i as routeLogsToStderr } from "./subsystem-BDbeCphF.js";
...
import { n as getSubCliEntries, r as registerSubCliByName } from "./register.subclis-DAJgItwI.js";
import { i as registerCoreCliByName, n as getCoreCliCommandNames } from "./command-registry-nxBYxAWJ.js";
import { t as getProgramContext } from "./program-context-CeOnIsy3.js";
...
async function completionCacheExists(shell, binName = "trident") {
...
function isCompletionProfileHeader(line) {
	return line.trim() === "# Trident Completion";
}
...
function updateCompletionProfile(content, binName, cachePath, sourceLine) {
	...
	const block = `# Trident Completion\n${sourceLine}`;
...
async function isCompletionInstalled(shell, binName = "trident") {
...
* Returns true if profile has `source <(trident completion ...)` instead of cached file.
*/
async function usesSlowDynamicCompletion(shell, binName = "trident") {
	const profilePath = getShellProfilePath(shell);
	if (!await pathExists(profilePath)) return false;
...
async function installCompletion(shell, yes, binName = "trident") {
	const home = process.env.HOME || os.homedir();
	let profilePath = "";
	let sourceLine = "";
