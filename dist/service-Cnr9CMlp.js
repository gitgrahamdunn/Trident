...
import { C as sleep } from "./utils-DwZbjiC4.js";
import { t as runCommandWithTimeout } from "./exec-BQevMHrc.js";
import { a as renderCmdSetAssignment, c as killProcessTree, i as parseCmdSetAssignment, n as quoteCmdScriptArg, p as inspectPortUsage, r as assertNoCmdLineBreak, s as findGatewayPidsOnPortSync, t as parseCmdScriptCommandLine } from "./cmd-argv-GXSSysSW.js";
import { d as resolveGatewayServiceDescription, p as resolveGatewayWindowsTaskName } from "./constants-D-pfuSKJ.js";
import { i as formatLine, n as resolveGatewayStateDir, o as writeFormattedLines, t as parseKeyValueOutput } from "./runtime-parse-Bb8Wi3Ut.js";
import { a as readLaunchAgentProgramArguments, d as stopLaunchAgent, f as uninstallLaunchAgent, o as readLaunchAgentRuntime, r as isLaunchAgentLoaded, t as installLaunchAgent, u as restartLaunchAgent } from "./launchd-C_0pZ8OL.js";
import { a as readSystemdServiceExecStart, c as restartSystemdService, d as uninstallSystemdService, l as stopSystemdService, o as readSystemdServiceRuntime, r as isSystemdServiceEnabled, t as installSystemdService } from "./systemd-ClrEDLHE.js";
...
function isGatewayArgv(args, opts) {
	...
	return exe.endsWith("/trident") || exe === "trident" || exe.endsWith("/openclaw") || exe === "openclaw" || opts?.allowGatewayBinary === true && (exe.endsWith("/trident-gateway") || exe.endsWith("/openclaw-gateway"));
}
//#endregion
...
function resolveTaskName(env) {
	const override = env.TRIDENT_WINDOWS_TASK_NAME?.trim() || env.OPENCLAW_WINDOWS_TASK_NAME?.trim();
	if (override) return override;
	return resolveGatewayWindowsTaskName(env.TRIDENT_PROFILE ?? env.OPENCLAW_PROFILE);
}
...
function resolveTaskScriptPath(env) {
	const override = env.TRIDENT_TASK_SCRIPT?.trim() || env.OPENCLAW_TASK_SCRIPT?.trim();
	if (override) return override;
	const scriptName = env.TRIDENT_TASK_SCRIPT_NAME?.trim() || env.OPENCLAW_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env) {
	const raw = env.TRIDENT_GATEWAY_PORT?.trim() || env.OPENCLAW_GATEWAY_PORT?.trim();
	if (!raw) return null;
...
async function resolveScheduledTaskPort(env) {
	...
	return parsePortFromProgramArguments(command?.programArguments) ?? parsePositivePort(command?.environment?.TRIDENT_GATEWAY_PORT) ?? parsePositivePort(command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(env);
}
