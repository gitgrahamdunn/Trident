...
import { $t as deliverOutboundPayloads, Ar as normalizeAccountId, At as hasNonzeroUsage, Br as normalizeChannelId, Ca as normalizeThinkLevel, Cr as emitSessionTranscriptUpdate, Ct as resolveAcpSessionCwd, Dr as updateSessionStore, Dt as AGENT_LANE_SUBAGENT, Ea as formatCliCommand, Et as AGENT_LANE_NESTED, Fr as resolveSessionResetType, Ft as normalizeReplyPayload, G as applyVerboseOverride, Gt as resolveOutboundTarget, H as setCliSessionId, Ir as mergeSessionEntry, Jt as formatDiscordUserTag, Kt as resolveSessionDeliveryTarget, Lr as setSessionRuntimeModel, Mr as evaluateSessionFreshness, Nn as clearBootstrapSnapshotOnSessionRollover, Nr as resolveChannelResetConfig, Ot as applyModelOverrideToSessionEntry, Pr as resolveSessionResetPolicy, Q as getSkillsSnapshotVersion, Qt as buildOutboundSessionContext, Rn as getAcpSessionManager, Rr as resolveExplicitAgentSessionKey, Sa as formatXHighModelHint, Sr as resolveSessionTranscriptFile, St as registerAgentRunContext, T as isDangerousNameMatchingEnabled, Ta as supportsXHighThinking, Tt as resolveAcpDispatchPolicyError, U as runCliAgent, Ui as buildWorkspaceSkillSnapshot, Ut as resolveAgentRoute, V as getCliSessionId, W as clearSessionAuthProfileOverride, Z as getRemoteSkillEligibility, _r as createMediaAttachmentCache, _t as resolveAgentTimeoutMs, a as modelKey, at as runEmbeddedPiAgent, ba as DEFAULT_PROVIDER, br as runCapability, bt as clearAgentRunContext, c as resolveConfiguredModelRef, ci as isGatewayMessageChannel, cn as FailoverError, cr as resolveTtsConfig, di as resolveMessageChannel, en as formatOutboundPayloadLog, et as resolveSendPolicy, f as getAgentRuntimeCommandSecretTargetIds, gr as buildProviderRegistry, i as isCliProvider, ia as loadModelCatalog, in as resolveMessageChannelSelection, jr as resolveSessionKey, jt as resolveContextTokensForModel, kt as deriveSessionTotalTokens, l as resolveDefaultModelForAgent, la as readConfigFileSnapshotForWrite, li as isInternalMessageChannel, lr as textToSpeech, nn as normalizeOutboundPayloadsForJson, nr as isSilentReplyText, o as normalizeModelRef, oa as loadConfig, oi as INTERNAL_MESSAGE_CHANNEL, on as runWithModelFallback, ot as prepareSessionManagerForRun, p as resolveCommandSecretRefsViaGateway, q as formatMention, qt as resolveDiscordOwnerAccess, r as buildAllowedModelSet, s as normalizeProviderId, si as isDeliverableMessageChannel, st as resolveBootstrapWarningSignaturesSeen, t as ensureAuthProfileStore, ta as formatErrorMessage, tn as normalizeOutboundPayloads, tr as isSilentReplyPrefixText, u as resolveThinkingDefault, ua as setRuntimeConfigSnapshot, ui as normalizeMessageChannel, ur as parseTtsDirectives, vr as normalizeMediaAttachments, vt as formatAgentInternalEventsForPrompt, wa as normalizeVerboseLevel, wr as loadSessionStore, wt as resolveAcpAgentPolicyError, xa as formatThinkingLevels, xt as emitAgentEvent, ya as DEFAULT_MODEL, yt as normalizeSpawnedRunMetadata, zn as toAcpRuntimeError, zr as getChannelPlugin } from "./auth-profiles-DDVivXkv.js";
...
function loadWhatsAppSenderRuntime() {
	whatsappSenderRuntimePromise ??= import("./deps-send-whatsapp.runtime-Dip72LcW.js");
	return whatsappSenderRuntimePromise;
}
function loadTelegramSenderRuntime() {
	telegramSenderRuntimePromise ??= import("./deps-send-telegram.runtime-BkEn-sXx.js");
	return telegramSenderRuntimePromise;
}
function loadDiscordSenderRuntime() {
	discordSenderRuntimePromise ??= import("./deps-send-discord.runtime-BiUh3AAj.js");
	return discordSenderRuntimePromise;
}
function loadSlackSenderRuntime() {
	slackSenderRuntimePromise ??= import("./deps-send-slack.runtime-jpQZVWMR.js");
	return slackSenderRuntimePromise;
}
function loadSignalSenderRuntime() {
	signalSenderRuntimePromise ??= import("./deps-send-signal.runtime-BvTqBOc7.js");
	return signalSenderRuntimePromise;
}
function loadIMessageSenderRuntime() {
	imessageSenderRuntimePromise ??= import("./deps-send-imessage.runtime-CunYalSG.js");
...
async function agentCommandInternal(opts, runtime = defaultRuntime, deps = createDefaultDeps()) {
	...
	try {
		...
		if (sessionStore && sessionKey && sessionEntry?.modelOverride?.trim()) {
			const overrideProvider = sessionEntry.providerOverride?.trim() || defaultProvider;
			const overrideModel = sessionEntry.modelOverride.trim();
			const runtimeProvider = sessionEntry.modelProvider?.trim() || "";
			const runtimeModel = sessionEntry.model?.trim() || "";
			const runtimeRef = runtimeProvider && runtimeModel ? `${runtimeProvider}/${runtimeModel}` : runtimeModel || runtimeProvider || "";
			const overrideRef = `${overrideProvider}/${overrideModel}`;
		...
		if (sessionEntry && sessionStore && sessionKey && hasStoredOverride) {
			...
			if (overrideModel) {
				...
				if (!isCliProvider(normalizedOverride.provider, cfg) && !allowAnyModel && !allowedModelKeys.has(key)) {
					const requestedRef = `${normalizedOverride.provider}/${normalizedOverride.model}`;
					const configuredRef = `${defaultProvider}/${defaultModel}`;
