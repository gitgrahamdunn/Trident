import { p as defaultRuntime, v as danger, y as info } from "./subsystem-BDbeCphF.js";
...
function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
	...
	browser.command("upload").description("Arm file upload for the next file chooser").argument("<paths...>", "File paths to upload (must be within the Trident temp uploads dir, e.g. /tmp/trident/uploads/file.pdf)").option("--ref <ref>", "Ref id from snapshot to click after arming").option("--input-ref <ref>", "Ref id for <input type=file> to set directly").option("--element <selector>", "CSS selector for <input type=file>").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next file chooser (default: 120000)", (v) => Number(v)).action(async (paths, opts, cmd) => {
		const { parent, profile } = resolveBrowserActionContext(cmd, parentOpts);
...
async function installChromeExtension(opts) {
	...
	if (!hasManifest(src)) throw new Error("Bundled Chrome extension is missing. Reinstall Trident and try again.");
...
function registerBrowserExtensionCommands(browser, parentOpts) {
	...
	ext.command("install").description("Install the Chrome extension to a stable local path").action(async (_opts, cmd) => {
		...
		defaultRuntime.error(info([
			...
			`- Pin “Trident Browser Relay”, then click it on the tab (badge shows ON)`,
			"",
			`${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.trident.ai/tools/chrome-extension")}`
		].join("\n")));
	});
	ext.command("path").description("Print the path to the installed Chrome extension (load unpacked)").action(async (_opts, cmd) => {
		...
		if (!hasManifest(dir)) {
			defaultRuntime.error(danger([`Chrome extension is not installed. Run: "${formatCliCommand("trident browser extension install")}"`, `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.trident.ai/tools/chrome-extension")}`].join("\n")));
			defaultRuntime.exit(1);
		}
		if (parent?.json) {
...
function registerBrowserCli(program) {
	const browser = program.command("browser").description("Manage Trident's dedicated browser (Chrome/Chromium)").option("--browser-profile <name>", "Browser profile name (default from config)").option("--json", "Output machine-readable JSON", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([...browserCoreExamples, ...browserActionExamples].map((cmd) => [cmd.replace(/^openclaw\b/, "trident"), ""]), true)}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/browser", "github.com/gitgrahamdunn/trident-cli/tree/main/docs/cli/browser")}\n`).action(() => {
		browser.outputHelp();
		defaultRuntime.error(danger(`Missing subcommand. Try: "${formatCliCommand("trident browser status")}"`));
		defaultRuntime.exit(1);
	});
