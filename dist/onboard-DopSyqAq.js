...
import { m as restoreTerminalState, p as defaultRuntime } from "./subsystem-D2xHvZZd.js";
import { y as resolveUserPath } from "./utils-DwZbjiC4.js";
import { At as randomToken, Ct as handleReset, Ft as waitForGatewayReachable, Tt as normalizeGatewayTokenInput, __ as writeConfigFile, _t as applyWizardMetadata, bt as ensureWorkspaceAndSessions, gt as DEFAULT_WORKSPACE, jt as resolveControlUiLinks, p_ as readConfigFileSnapshot } from "./reply-Bm8VrLQh.js";
...
import { jt as resolveDefaultSecretProviderAlias } from "./plugins-Csb211Yn.js";
...
import "./daemon-runtime-BD6CXAJV.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-Cnr9CMlp.js").then((n) => n.r);
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-DTU6Q8pZ.js").then((n) => n.t);
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-CWCuUaps.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-JrWgVYUX.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-DF6LhIsa.js").then((n) => n.i);
