...
function formatAmbiguousGatewayAuthModeReason() {
	return ["gateway.auth.token and gateway.auth.password are both configured while gateway.auth.mode is unset.", `Set ${formatCliCommand("trident config set gateway.auth.mode token")} or ${formatCliCommand("trident config set gateway.auth.mode password")}.`].join(" ");
}
...
function throwBootstrapGuiSessionError(params) {
	throw new Error([
		...
		"Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.trident.ai/gateway"
	].join("\n"));
}
...
async function installLaunchAgent({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	await bootstrapLaunchAgentOrThrow({
		...
		serviceTarget: `${domain}/${label}`,
		plistPath,
		actionHint: "trident gateway install --force"
	});
...
async function restartLaunchAgent({ stdout, env }) {
	...
	await bootstrapLaunchAgentOrThrow({
		...
		serviceTarget,
		plistPath,
		actionHint: "trident gateway restart"
	});
...
function buildSystemdUnit({ description, programArguments, workingDirectory, environment }) {
	...
	const descriptionValue = description?.trim() || "Trident Gateway";
...
function renderGatewayServiceStartHints(env = process.env) {
	...
	return buildPlatformServiceStartHints({
		installCommand: formatCliCommand("trident gateway install", env),
		startCommand: formatCliCommand("trident gateway", env),
	...
	if (loaded) {
		if (!opts.force) {
			...
			if (!json) {
				...
				defaultRuntime.log(`Reinstall with: ${formatCliCommand("trident gateway install --force")}`);
			}
			return;
		}
	}
...
function auditGatewayToken(command, issues, expectedGatewayToken) {
	...
	issues.push({
		...
		detail: "Run `trident gateway install --force` to remove embedded service token.",
		level: "recommended"
	});
	...
	issues.push({
		...
		message: "Gateway service OPENCLAW_GATEWAY_TOKEN does not match gateway.auth.token in trident.json",
...
function checkTokenDrift(params) {
	...
	if (configToken && serviceToken !== configToken) return {
		...
		detail: "Run `trident gateway install --force` to sync the token.",
		level: "recommended"
	};
	return null;
}
...
function renderSystemdUnavailableHints(options = {}) {
	...
	return ["systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.", `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("trident gateway")}\`.`];
}
//#endregion
...
async function runServiceStart(params) {
	...
	}
	{
		...
		if (configError) {
			fail(`${params.serviceNoun} aborted: config is invalid.\n${configError}\nFix the config and retry, or run "trident doctor" to repair.`);
			return;
		}
	}
	try {
...
async function restartGatewayWithoutServiceManager(port) {
	...
	const pids = resolveVerifiedGatewayListenerPids(port);
	if (pids.length === 0) return null;
	if (pids.length > 1) throw new Error(`multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "trident gateway status --deep" before retrying restart`);
	...
	return await runServiceRestart({
		...
		postRestartCheck: async ({ warnings, fail, stdout }) => {
			if (restartedWithoutServiceManager) {
				...
				fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [formatCliCommand("trident gateway status --deep"), formatCliCommand("trident doctor")]);
			}
...
function printDaemonStatus(status, opts) {
	...
	if (service.configAudit?.issues.length) {
		...
		defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("trident doctor")}" (or "${formatCliCommand("trident doctor --repair")}").`));
	}
	if (status.config) {
		...
		if (status.config.mismatch) {
			...
			defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("trident gateway install --force")}\` from the same --profile / OPENCLAW_STATE_DIR you expect.`));
		}
		spacer();
	}
	if (status.gateway) {
	...
	if (service.runtime?.cachedLabel) {
		...
		defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("trident gateway install")}`));
		spacer();
	}
	...
	defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("trident status")}`);
	defaultRuntime.log(`${label("Troubleshooting:")} https://docs.trident.ai/troubleshooting`);
}
//#endregion
