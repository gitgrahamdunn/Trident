import { p as defaultRuntime, t as createSubsystemLogger } from "./subsystem-BDbeCphF.js";
import { $i as resolveContextTokensForModel, Af as resolveSessionTranscriptFile, Ar as resolveSendPolicy, Av as normalizeThinkLevel, C as isCliProvider, Cg as loadModelCatalog, Ci as emitAgentEvent, Cn as setCliSessionId, Cv as formatThinkingLevels, Di as resolveAcpSessionCwd, E as normalizeModelRef, Ei as registerAgentRunContext, Ff as loadSessionStore, Fg as readConfigFileSnapshotForWrite, Fp as isInternalMessageChannel, Ga as formatOutboundPayloadLog, Gr as resolveBootstrapWarningSignaturesSeen, Ha as buildOutboundSessionContext, Ii as applyModelOverrideToSessionEntry, K as getAgentRuntimeCommandSecretTargetIds, Ka as normalizeOutboundPayloads, L as resolveThinkingDefault, Lg as setRuntimeConfigSnapshot, Lp as normalizeMessageChannel, Lr as runEmbeddedPiAgent, M as resolveConfiguredModelRef, Mi as AGENT_LANE_NESTED, Mp as isDeliverableMessageChannel, Mv as normalizeVerboseLevel, N as resolveDefaultModelForAgent, Na as resolveOutboundTarget, Nf as emitSessionTranscriptUpdate, Ni as AGENT_LANE_SUBAGENT, O as normalizeProviderId, Oi as resolveAcpAgentPolicyError, Pa as resolveSessionDeliveryTarget, Pp as isGatewayMessageChannel, Pv as supportsXHighThinking, Rd as isSilentReplyPrefixText, Rp as resolveMessageChannel, Rr as prepareSessionManagerForRun, Si as clearAgentRunContext, Sn as getCliSessionId, Sv as DEFAULT_PROVIDER, T as modelKey, Tn as clearSessionAuthProfileOverride, Ua as deliverOutboundPayloads, Vn as getRemoteSkillEligibility, Wu as clearBootstrapSnapshotOnSessionRollover, Z as resolveCommandSecretRefsViaGateway, _p as resolveExplicitAgentSessionKey, _r as getSkillsSnapshotVersion, ao as resolveMessageChannelSelection, bi as normalizeSpawnedRunMetadata, co as runWithModelFallback, cp as evaluateSessionFreshness, do as FailoverError, dp as resolveSessionResetType, fp as mergeSessionEntry, hd as toAcpRuntimeError, ia as normalizeReplyPayload, jg as loadConfig, jn as applyVerboseOverride, jp as INTERNAL_MESSAGE_CHANNEL, ki as resolveAcpDispatchPolicyError, lp as resolveChannelResetConfig, md as getAcpSessionManager, mp as setSessionRuntimeModel, p as ensureAuthProfileStore, qa as normalizeOutboundPayloadsForJson, qf as normalizeAccountId, sp as resolveSessionKey, up as resolveSessionResetPolicy, vi as resolveAgentTimeoutMs, wn as runCliAgent, wv as formatXHighModelHint, xv as DEFAULT_MODEL, y as buildAllowedModelSet, yi as formatAgentInternalEventsForPrompt, zd as isSilentReplyText, zf as updateSessionStore } from "./auth-profiles-DRjqKE3G.js";
import { t as formatCliCommand } from "./command-format-BTnLVWI8.js";
import { D as ensureAgentWorkspace, a as resolveAgentDir, d as resolveAgentWorkspaceDir, h as resolveSessionAgentId, p as resolveEffectiveModelFallbacks, r as listAgentIds, u as resolveAgentSkillsFilter } from "./agent-scope-CZIF93u7.js";
import { c as normalizeAgentId, l as normalizeMainKey, u as resolveAgentIdFromSessionKey } from "./session-key-51LnISpq.js";
import { r as buildWorkspaceSkillSnapshot } from "./skills-CtzUimzY.js";
import { r as normalizeChannelId, t as getChannelPlugin } from "./plugins-C-z6VZlO.js";
import { l as resolveStorePath } from "./paths-B9q4Ev3T.js";
import { d as deriveSessionTotalTokens, f as hasNonzeroUsage } from "./session-cost-usage-B7YSslo_.js";
import { n as createDefaultDeps, t as createOutboundSendDeps } from "./outbound-send-deps-CzhlcoLa.js";
...
function formatSessionModelRef(provider, model) {
	const normalizedProvider = typeof provider === "string" ? provider.trim() : "";
	const normalizedModel = typeof model === "string" ? model.trim() : "";
	if (normalizedProvider && normalizedModel) return `${normalizedProvider}/${normalizedModel}`;
	return normalizedModel || normalizedProvider || void 0;
}
function prependInternalEventContext(body, events) {
	if (body.includes("Trident runtime context (internal):")) return body;
...
async function prepareAgentCommandExecution(opts, runtime) {
	...
	if (agentIdOverride) {
		if (!listAgentIds(cfg).includes(agentIdOverride)) throw new Error(`Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("trident agents list")}" to see configured agents.`);
	}
...
async function agentCommandInternal(opts, runtime = defaultRuntime, deps = createDefaultDeps()) {
	...
	try {
		...
		const sessionReuseLabel = isNewSession ? "new" : "reused";
		const previousPersistedModel = formatSessionModelRef(sessionEntry?.modelProvider, sessionEntry?.model) ?? formatSessionModelRef(sessionEntry?.providerOverride, sessionEntry?.modelOverride);
		const requestedModelRef = formatSessionModelRef(sessionEntry?.providerOverride, sessionEntry?.modelOverride);
		if (!isNewSession && sessionEntry?.modelOverride?.trim()) {
			const overrideProvider = sessionEntry.providerOverride?.trim() || defaultProvider;
			const overrideModel = sessionEntry.modelOverride.trim();
			const runtimeProvider = sessionEntry.modelProvider?.trim() || "";
			const runtimeModel = sessionEntry.model?.trim() || "";
			const runtimeRef = formatSessionModelRef(runtimeProvider, runtimeModel);
			const overrideRef = formatSessionModelRef(overrideProvider, overrideModel);
			const staleRuntime = runtimeModel && runtimeRef !== overrideRef;
			if (staleRuntime && sessionStore && sessionKey) {
				const next = {
					...sessionEntry,
					updatedAt: Date.now()
				};
		...
		if (sessionEntry && sessionStore && sessionKey && hasStoredOverride) {
			...
			if (overrideModel) {
				...
				if (!isCliProvider(normalizedOverride.provider, cfg) && !allowAnyModel && !allowedModelKeys.has(key)) {
					const requestedRef = formatSessionModelRef(normalizedOverride.provider, normalizedOverride.model) ?? `${normalizedOverride.provider}/${normalizedOverride.model}`;
					const configuredRef = formatSessionModelRef(defaultProvider, defaultModel) ?? `${defaultProvider}/${defaultModel}`;
					log.error(`[subagent:model-invalid] session=${sessionKey} requested=${requestedRef} configured=${configuredRef} reason=unavailable_or_disallowed`);
					throw new Error(`Requested model "${requestedRef}" is unavailable or disallowed for session ${sessionKey}. Configured primary is "${configuredRef}". Branch failed loudly instead of silently falling back.`);
				}
			}
		}
		...
		log.warn(`[subagent:session-model] session=${sessionKey ?? sessionId} state=${sessionReuseLabel} requested=${requestedModelRef ?? "<configured>"} previous=${previousPersistedModel ?? "<none>"} final=${provider}/${model}`);
		if (sessionEntry) {
		...
		try {
			...
			if (opts.lane === AGENT_LANE_SUBAGENT || storedModelOverride) log.warn(`[agent:model] session=${sessionKey ?? sessionId} requested=${storedModelOverride ? `${storedProviderOverride || defaultProvider}/${storedModelOverride}` : "<configured>"} effective=${provider}/${model}`);
