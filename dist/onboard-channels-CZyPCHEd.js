...
import { Aa as normalizeIMessageHandle, Cp as resolveDefaultSignalAccountId, Ia as normalizeDiscordSlug, Lt as resolveDiscordChannelAllowlist, Nu as createPluginLoaderLogger, Qa as isChannelConfigured, Sp as listSignalAccountIds, br as resolveDiscordUserAllowlist, ht as detectBinary, ot as resolveSlackChannelAllowlist, to as listChannelPluginCatalogEntries, wp as resolveSignalAccount, yr as resolveSlackUserAllowlist, z as loadOpenClawPlugins } from "./auth-profiles-DRjqKE3G.js";
...
import { _ as normalizeAccountId, g as DEFAULT_ACCOUNT_ID } from "./session-key-51LnISpq.js";
import { m as pathExists, p as normalizeE164, t as CONFIG_DIR } from "./utils-B88a096J.js";
...
import { i as formatChannelSelectionLine, o as listChatChannels, r as formatChannelPrimerLine } from "./registry-DtTKJfN8.js";
import { g as clearPluginDiscoveryCache } from "./skills-CtzUimzY.js";
import { D as resolveDefaultSlackAccountId, E as listSlackAccountIds, H as resolveDefaultWhatsAppAccountId, O as resolveSlackAccount, T as inspectSlackAccount, V as listWhatsAppAccountIds, W as resolveWhatsAppAuthDir, _t as resolveDiscordAccount, a as inspectTelegramAccount, at as resolveDefaultIMessageAccountId, c as listTelegramAccountIds, dt as inspectDiscordAccount, gt as resolveDefaultDiscordAccountId, it as listIMessageAccountIds, l as resolveDefaultTelegramAccountId, n as listChannelPlugins, ot as resolveIMessageAccount, pt as listDiscordAccountIds, t as getChannelPlugin, u as resolveTelegramAccount, ut as resolveChannelDefaultAccountId } from "./plugins-C-z6VZlO.js";
import { t as formatDocsLink } from "./links-BuWtQKK5.js";
import { t as loginWeb } from "./login-CC_UlAJF.js";
import { t as resolveBrewExecutable } from "./brew-BqnB6wF-.js";
import { n as moveSingleAccountChannelSectionToDefaultAccount, r as patchScopedAccountConfig, t as fetchTelegramChatId } from "./api-CVGd6c_4.js";
import { t as enablePluginInConfig } from "./enable-C_k_lH3p.js";
import { s as extractArchive } from "./install-safe-path-DZM2JHcC.js";
import { a as resolveBundledPluginSources, i as findBundledPluginSourceInMap, n as recordPluginInstall, s as installPluginFromNpmSpec, t as buildNpmResolutionInstallFields } from "./installs-K3NXKFo2.js";
import { n as resolveBundledInstallPlanForCatalogEntry } from "./plugin-install-plan-KM-lV5Yn.js";
import { c as resolveSecretInputModeForEnvSelection, s as promptSecretRefForOnboarding } from "./auth-choice.apply-helpers-Bcqf2um4.js";
...
async function promptSingleChannelSecretInput(params) {
	if (await resolveSecretInputModeForEnvSelection({
		...
		copy: {
			...
			plaintextHint: "Stores the credential directly in Trident config",
...
const imessageOnboardingAdapter = {
	...
	configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
		...
		await prompter.note([
			...
			"Ensure Trident has Full Disk Access to Messages DB.",
...
const signalOnboardingAdapter = {
	...
	configure: async ({ cfg, runtime, prompter, accountOverrides, shouldPromptAccountIds, options }) => {
		...
		await prompter.note([
			"Link device with: signal-cli link -n \"Trident\"",
			"Scan QR in Signal → Linked Devices",
			`Then run: ${formatCliCommand("trident gateway call channels.status --params '{\"probe\":true}'")}`,
			`Docs: ${formatDocsLink("/signal", "signal")}`
		].join("\n"), "Signal next steps");
		return {
			cfg: next,
...
function buildSlackManifest(botName) {
	const safeName = botName.trim() || "Trident";
	const manifest = {
		display_information: {
			name: safeName,
			description: `${safeName} connector for Trident`
		},
		features: {
			bot_user: {
			...
			slash_commands: [{
				command: "/openclaw",
				description: "Send a message to Trident",
				should_escape: false
			}]
		},
...
async function noteTelegramUserIdHelp(prompter) {
	await prompter.note([
		`1) DM your bot, then read from.id in \`${formatCliCommand("trident logs --follow")}\` (safest)`,
...
async function promptWhatsAppOwnerAllowFrom(params) {
	...
	await prompter.note("We need the sender/owner number so Trident can allowlist you.", "WhatsApp number");
...
async function promptWhatsAppAllowFrom(cfg, _runtime, prompter, options) {
	...
	if (await prompter.select({
		...
		options: [{
			...
			value: "separate",
			label: "Separate phone just for Trident"
		}]
...
const BUILTIN_ONBOARDING_ADAPTERS = [
	telegramOnboardingAdapter,
	{
		...
		configure: async ({ cfg, runtime, prompter, options, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
			...
			else if (!linked) await prompter.note(`Run \`${formatCliCommand("trident channels login")}\` later to link WhatsApp.`, "WhatsApp");
...
async function noteChannelPrimer(prompter, channels) {
	...
	await prompter.note([
		...
		`Approve with: ${formatCliCommand("trident pairing approve <channel> <code>")}`,
		"Public DMs require dmPolicy=\"open\" + allowFrom=[\"*\"].",
		"Multi-user DMs: run: " + formatCliCommand("trident config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.",
	...
	const ensureBundledPluginEnabled = async (channel) => {
		...
		if (!getChannelPlugin(channel)) {
			if (getChannelOnboardingAdapter(channel)) {
				await prompter.note(`${channel} plugin not available (continuing with onboarding). If the channel still doesn't work after setup, run \`${formatCliCommand("trident plugins list")}\` and \`${formatCliCommand("trident plugins enable " + channel)}\`, then restart the gateway.`, "Channel setup");
	...
	if (options?.quickstartDefaults) {
		...
		const choice = await prompter.select({
			...
			options: [...buildSelectionOptions(entries), {
				value: "__skip__",
				label: "Skip for now",
				hint: `You can add channels later via \`${formatCliCommand("trident channels add")}\``
			}],
