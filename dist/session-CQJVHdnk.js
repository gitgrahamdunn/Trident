import { E as success, O as getChildLogger, j as toPinoLikeLogger, v as danger } from "./subsystem-D2xHvZZd.js";
import { c as ensureDir, y as resolveUserPath } from "./utils-DwZbjiC4.js";
import { uv as VERSION } from "./reply-Bm8VrLQh.js";
import { t as formatCliCommand } from "./command-format-3Z_Kl5PP.js";
import { Q as readCredsJsonRaw, X as maybeRestoreCredsFromBackup, et as resolveDefaultWebAuthDir, nt as resolveWebCredsPath, tt as resolveWebCredsBackupPath } from "./plugins-Csb211Yn.js";
...
async function createWaSocket(printQr, verbose, opts = {}) {
	...
	sock.ev.on("connection.update", (update) => {
		try {
			...
			if (connection === "close") {
				if (getStatusCode(lastDisconnect?.error) === DisconnectReason.loggedOut) console.error(danger(`WhatsApp session logged out. Run: ${formatCliCommand("trident channels login")}`));
			}
