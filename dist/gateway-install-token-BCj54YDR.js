import { Ch as readGatewayTokenEnv, Dt as randomToken, Pg as readConfigFileSnapshot, Rg as writeConfigFile, T_ as collectConfigServiceEnvVars, Zm as resolveSecretRefValues, h as loadAuthProfileStoreForSecretsRuntime, ph as hasAmbiguousGatewayAuthModeConfig, vh as resolveGatewayAuth } from "./auth-profiles-DRjqKE3G.js";
...
import { Mt as secretRefKey } from "./plugins-C-z6VZlO.js";
import { l as resolveGatewayLaunchAgentLabel } from "./constants-CfAbrI_x.js";
import { f as resolveGatewayProgramArguments, n as resolveDaemonInstallRuntimeInputs, t as emitDaemonInstallRuntimeWarning, u as buildServiceEnvironment } from "./daemon-install-plan.shared-C827UpZv.js";
...
function gatewayInstallErrorHint(platform = process.platform) {
	return platform === "win32" ? "Tip: native Windows now falls back to a per-user Startup-folder login item when Scheduled Task creation is denied; if install still fails, rerun from an elevated PowerShell or skip service install." : `Tip: rerun \`${formatCliCommand("trident gateway install")}\` after fixing the error.`;
}
//#endregion
