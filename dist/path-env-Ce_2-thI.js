...
import { n as resolveBrewPathDirs } from "./brew-BoTYTRV7.js";
...
function candidateBinDirs(opts) {
	...
	try {
		...
		if (isExecutable(path.join(execDir, "trident")) || isExecutable(path.join(execDir, "openclaw"))) prepend.push(execDir);
	} catch {}
	if (opts.allowProjectLocalBin === true || isTruthyEnvValue(process.env.OPENCLAW_ALLOW_PROJECT_LOCAL_BIN)) {
		const localBinDir = path.join(cwd, "node_modules", ".bin");
		if (isExecutable(path.join(localBinDir, "trident")) || isExecutable(path.join(localBinDir, "openclaw"))) append.push(localBinDir);
	}
