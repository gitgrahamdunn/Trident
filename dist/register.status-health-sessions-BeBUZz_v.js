...
import { p as defaultRuntime, w as setVerbose, y as info } from "./subsystem-D2xHvZZd.js";
import { n as isRich, r as theme } from "./theme-CipOb_We.js";
import "./utils-DwZbjiC4.js";
import { Cp as resolveMaintenanceConfig, Fi as lookupContextTokens, P as resolveConfiguredModelRef, Pi as formatTimeAgo, Sp as pruneStaleEntries, Yp as resolveFreshSessionTotalTokens, bp as updateSessionStore, gp as loadSessionStore, ji as resolveSessionModelRef, lp as resolveSessionStoreTargets, q as formatHelpExamples, u_ as loadConfig, wi as classifySessionKey, wp as enforceSessionDiskBudget, xp as capEntryCount } from "./reply-Bm8VrLQh.js";
...
import { i as resolveSessionFilePathOptions, r as resolveSessionFilePath } from "./paths-DBVuZIGo.js";
...
import { n as parsePositiveIntOrUndefined } from "./helpers-DjgA-EwS.js";
import "./skills-status-CitUjZFm.js";
import "./dangerous-tools-BmJSxtj-.js";
...
import { t as statusCommand } from "./status-B4IXgQ9e.js";
import { r as healthCommand } from "./health-DF6LhIsa.js";
import "./channel-account-context-BvCbqI5G.js";
import "./node-command-policy-CBt-HNiN.js";
import "./audit-B3LIgyiT.js";
...
function registerStatusHealthSessionsCommands(program) {
	program.command("status").description("Show channel health and recent session recipients").option("--json", "Output JSON instead of text", false).option("--all", "Full diagnosis (read-only, pasteable)", false).option("--usage", "Show model provider usage/quota snapshots", false).option("--deep", "Probe channels (WhatsApp Web + Telegram + Discord + Slack + Signal)", false).option("--timeout <ms>", "Probe timeout in milliseconds", "10000").option("--verbose", "Verbose logging", false).option("--debug", "Alias for --verbose", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["trident status", "Show channel health + session summary."],
		["trident status --all", "Full diagnosis (read-only)."],
		["trident status --json", "Machine-readable output."],
		["trident status --usage", "Show model provider usage/quota snapshots."],
		["trident status --deep", "Run channel probes (WA + Telegram + Discord + Slack + Signal)."],
		["trident status --deep --timeout 5000", "Tighten probe timeout."]
	])}`).addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.trident.ai/cli/status")}\n`).action(async (opts) => {
	...
	const sessionsCmd = program.command("sessions").description("List stored conversation sessions").option("--json", "Output as JSON", false).option("--verbose", "Verbose logging", false).option("--store <path>", "Path to session store (default: resolved from config)").option("--agent <id>", "Agent id to inspect (default: configured default agent)").option("--all-agents", "Aggregate sessions across all configured agents", false).option("--active <minutes>", "Only show sessions updated within the past N minutes").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["trident sessions", "List all sessions."],
		["trident sessions --agent work", "List sessions for one agent."],
		["trident sessions --all-agents", "Aggregate sessions across agents."],
		["trident sessions --active 120", "Only last 2 hours."],
		["trident sessions --json", "Machine-readable output."],
		["trident sessions --store ./tmp/sessions.json", "Use a specific session store."]
	])}\n\n${theme.muted("Shows token usage per session when the agent reports it; set agents.defaults.contextTokens to cap the window and show %.")}`).addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.trident.ai/cli/sessions")}\n`).action(async (opts) => {
	...
	sessionsCmd.command("cleanup").description("Run session-store maintenance now").option("--store <path>", "Path to session store (default: resolved from config)").option("--agent <id>", "Agent id to maintain (default: configured default agent)").option("--all-agents", "Run maintenance across all configured agents", false).option("--dry-run", "Preview maintenance actions without writing", false).option("--enforce", "Apply maintenance even when configured mode is warn", false).option("--fix-missing", "Remove store entries whose transcript files are missing (bypasses age/count retention)", false).option("--active-key <key>", "Protect this session key from budget-eviction").option("--json", "Output JSON", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["trident sessions cleanup --dry-run", "Preview stale/cap cleanup."],
		["trident sessions cleanup --dry-run --fix-missing", "Also preview pruning entries with missing transcript files."],
		["trident sessions cleanup --enforce", "Apply maintenance now."],
		["trident sessions cleanup --agent work --dry-run", "Preview one agent store."],
		["trident sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
		["trident sessions cleanup --enforce --store ./tmp/sessions.json", "Use a specific store."]
