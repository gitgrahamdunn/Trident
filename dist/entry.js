#!/usr/bin/env node
import { J as hasHelpOrVersion, X as isRootHelpInvocation, Z as isRootVersionInvocation, t as createSubsystemLogger } from "./subsystem-BDbeCphF.js";
...
function resolveProfileStateDir(profile, env, homedir) {
	...
	return path.join(resolveRequiredHomeDir(env, homedir), `.trident${suffix}`);
}
function applyCliProfileEnv(params) {
	...
	if (!profile) return;
	env.TRIDENT_PROFILE = profile;
	env.OPENCLAW_PROFILE = profile;
	const stateDir = env.TRIDENT_STATE_DIR?.trim() || env.OPENCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
	if (!env.TRIDENT_STATE_DIR?.trim()) env.TRIDENT_STATE_DIR = stateDir;
	if (!env.OPENCLAW_STATE_DIR?.trim()) env.OPENCLAW_STATE_DIR = stateDir;
	if (!env.TRIDENT_CONFIG_PATH?.trim()) env.TRIDENT_CONFIG_PATH = path.join(stateDir, "trident.json");
	if (!env.OPENCLAW_CONFIG_PATH?.trim()) env.OPENCLAW_CONFIG_PATH = path.join(stateDir, "trident.json");
...
if (!isMainModule({
	...
	function ensureExperimentalWarningSuppressed() {
		...
		child.once("error", (error) => {
			console.error("[trident] Failed to respawn CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exit(1);
		});
		return true;
	}
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-DcA9ITyc.js").then((n) => n.r), import("./git-commit-B0vIx0co.js").then((n) => n.t)]).then(([{ VERSION }, { resolveCommitHash }]) => {
			const commit = resolveCommitHash({ moduleUrl: import.meta.url });
			console.log(commit ? `Trident ${VERSION} (${commit})` : `Trident ${VERSION}`);
			process$1.exit(0);
		}).catch((error) => {
			console.error("[trident] Failed to resolve version:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
		return true;
	}
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-DdPIyEwa.js").then(({ buildProgram }) => {
			buildProgram().outputHelp();
		}).catch((error) => {
			console.error("[trident] Failed to display help:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
		return true;
	}
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!parsed.ok) {
			console.error(`[trident] ${parsed.error}`);
			process$1.exit(2);
		}
		if (parsed.profile) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-Cu0soMcD.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
			console.error("[trident] Failed to start CLI:", error instanceof Error ? error.stack ?? error.message : error);
			process$1.exitCode = 1;
		});
	}
}
