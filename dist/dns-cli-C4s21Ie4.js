...
function registerDnsCli(program) {
	program.command("dns").description("DNS helpers for wide-area discovery (Tailscale + CoreDNS)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.trident.ai/cli/dns")}\n`).command("setup").description("Set up CoreDNS to serve your discovery domain for unicast DNS-SD (Wide-Area Bonjour)").option("--domain <domain>", "Wide-area discovery domain (e.g. trident.internal)").option("--apply", "Install/update CoreDNS config and (re)start the service (requires sudo)", false).action(async (opts) => {
		...
		defaultRuntime.log("");
		defaultRuntime.log(theme.heading("Recommended ~/.trident/trident.json:"));
		...
		if (zoneFileNeedsBootstrap(zonePath)) {
			...
			const zoneLines = [
				`; created by trident dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
		...
		if (cfg.discovery?.wideArea?.enabled !== true) {
			defaultRuntime.log("");
			defaultRuntime.log(theme.muted("Note: enable discovery.wideArea.enabled in ~/.trident/trident.json on the gateway and restart the gateway so it writes the DNS-SD zone."));
		}
	});
}
//#endregion
