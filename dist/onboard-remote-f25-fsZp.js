...
import { Nh as isSecureWebSocketUrl, ht as detectBinary } from "./auth-profiles-DRjqKE3G.js";
...
import { n as resolveWideAreaDiscoveryDomain } from "./widearea-dns-ZL3DJHax.js";
...
async function promptRemoteGatewayConfig(cfg, prompter, options) {
	...
	if (!hasBonjourTool) await prompter.note(["Bonjour discovery requires dns-sd (macOS) or avahi-browse (Linux).", "Docs: https://docs.trident.ai/gateway/discovery"].join("\n"), "Discovery");
	if (wantsDiscover) {
	...
	if (authChoice === "token") {
		if (await resolveSecretInputModeForEnvSelection({
			...
			copy: {
				...
				plaintextHint: "Stores the token directly in Trident config"
			}
		...
		if (await resolveSecretInputModeForEnvSelection({
			...
			copy: {
				...
				plaintextHint: "Stores the password directly in Trident config"
			}
