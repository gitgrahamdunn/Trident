...
import { G as formatHelpExamples, Oo as trimLogTail, Pg as readConfigFileSnapshot, Rg as writeConfigFile, ks as formatDurationPrecise } from "./auth-profiles-DRjqKE3G.js";
import { n as replaceCliName, r as resolveCliName, t as formatCliCommand } from "./command-format-BTnLVWI8.js";
import "./agent-scope-CZIF93u7.js";
import { m as pathExists } from "./utils-B88a096J.js";
import "./boundary-file-read-Bb0WDUIN.js";
import "./logger-D0msxOz2.js";
import { t as runCommandWithTimeout } from "./exec-CdZJtviz.js";
import { t as resolveOpenClawPackageRoot } from "./trident-root-CbcBJZgO.js";
...
import { n as quoteCmdScriptArg } from "./cmd-argv-Cnl266PT.js";
import "./delivery-queue-uotfwRH3.js";
import "./paths-B9q4Ev3T.js";
import "./session-cost-usage-B7YSslo_.js";
import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName } from "./constants-CfAbrI_x.js";
import { n as stylePromptMessage } from "./prompt-style-C94HXvSH.js";
...
import { r as installCompletion } from "./completion-cli-DHtFpvB3.js";
import "./register.subclis-DAJgItwI.js";
import "./command-registry-nxBYxAWJ.js";
import "./program-context-CeOnIsy3.js";
import "./daemon-install-plan.shared-C827UpZv.js";
import { r as parseSemver } from "./runtime-guard-BGuMyQeT.js";
import "./gateway-install-token-BCj54YDR.js";
import "./runtime-parse-BCRWcre9.js";
import "./launchd-DnTV_bc7.js";
import { n as resolveGatewayService } from "./service-B6O2Y9L1.js";
import "./systemd---Z1c197.js";
import "./lifecycle-core-BWGEd8ra.js";
import "./systemd-hints-C5pH3Xds.js";
import { a as terminateStaleGatewayPids, i as renderRestartDiagnostics, o as waitForGatewayHealthyRestart, r as runDaemonRestart, s as runDaemonInstall } from "./daemon-cli-Cfj5GfCC.js";
...
import { d as formatUpdateChannelLabel, g as resolveUpdateChannelDisplay, h as resolveEffectiveUpdateChannel, i as fetchNpmTagVersion, m as normalizeUpdateChannel, n as checkUpdateStatus, o as resolveNpmChannelTag, r as compareSemverStrings, u as channelToNpmTag } from "./channel-account-context-D_0axdYi.js";
import { a as detectGlobalInstallManagerForRoot, c as resolveGlobalPackageRoot, d as readPackageVersion, i as detectGlobalInstallManagerByPresence, l as normalizePackageTagInput, n as cleanupGlobalRenameDirs, o as globalInstallArgs, r as createGlobalInstallEnv, s as resolveGlobalInstallSpec, t as runGatewayUpdate, u as readPackageName } from "./update-runner-DwRB0HyE.js";
import { i as resolveUpdateAvailability, n as formatUpdateOneLiner, t as formatUpdateAvailableHint } from "./status.update-BDMAkJyU.js";
import "./logging-DRp8gftI.js";
import "./oauth-tls-preflight-BgDtqYZW.js";
import { n as updateNpmInstalledPlugins, t as syncPluginsForUpdateChannel } from "./update-9MUEa2z_.js";
import "./doctor-config-flow-DM9Q7QuP.js";
import "./systemd-linger-CdgSjHC6.js";
import "./health-format-n3gqKELa.js";
import { n as doctorCommand, t as selectStyled } from "./prompt-select-styled-BhPJUfYZ.js";
import { r as ensureCompletionCacheExists, t as checkShellCompletionStatus } from "./doctor-completion-LbNI8mVm.js";
...
const OPENCLAW_REPO_URL = "https://github.com/trident/trident.git";
const MAX_LOG_CHARS = 8e3;
const DEFAULT_PACKAGE_NAME = "trident";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
function normalizeTag(value) {
	return normalizePackageTagInput(value, ["trident", DEFAULT_PACKAGE_NAME]);
}
...
async function ensureGitCheckout(params) {
	...
	if (!await isGitCheckout(params.dir)) {
		if (!await isEmptyDir(params.dir)) throw new Error(`OPENCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAW_GIT_DIR to an empty folder or an trident checkout.`);
...
async function tryWriteCompletionCache(root, jsonMode) {
	const binPath = path.join(root, "trident.mjs");
...
async function updateStatusCommand(opts) {
	...
	defaultRuntime.log(theme.heading("Trident update status"));
	defaultRuntime.log("");
...
const STEP_LABELS = {
	...
	"trident doctor entry": "Checking doctor entrypoint",
	"trident doctor": "Running doctor checks",
...
function inferUpdateFailureHints(result) {
	...
	if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
		...
		hints.push("Example: npm config set prefix ~/.local && npm i -g trident@latest");
	}
	if (failedStep.name.startsWith("global update") && (stderr.includes("node-gyp") || stderr.includes("prebuild"))) {
		...
		hints.push("If it still fails: npm i -g trident@latest --omit=optional");
	}
	return hints;
}
...
async function prepareRestartScript(env = process.env, gatewayPort = DEFAULT_GATEWAY_PORT) {
	...
	try {
		if (platform === "linux") {
			...
			filename = `trident-restart-${timestamp}.sh`;
...
`;
			...
			filename = `trident-restart-${timestamp}.bat`;
...
async function tryInstallShellCompletion(opts) {
	...
	if (!status.profileInstalled) {
		...
		if (isCancel(shouldInstall) || !shouldInstall) {
			if (!opts.skipPrompt) defaultRuntime.log(theme.muted(`Skipped. Run \`${replaceCliName(formatCliCommand("trident completion --install"), CLI_NAME)}\` later to enable.`));
			return;
		}
...
async function runPackageInstallUpdate(params) {
	...
	const packageName = (pkgRoot ? await readPackageName(pkgRoot) : await readPackageName(params.root)) ?? "trident";
...
async function maybeRestartService(params) {
		...
		try {
			...
			if (!params.opts.json && restartInitiated) {
				...
				if (health.healthy) defaultRuntime.log(theme.success("Daemon restart completed."));
					...
					defaultRuntime.log(theme.muted(`Run \`${replaceCliName(formatCliCommand("trident gateway status --deep"), CLI_NAME)}\` for details.`));
				}
				defaultRuntime.log("");
			}
		} catch (err) {
			if (!params.opts.json) {
				...
				defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${replaceCliName(formatCliCommand("trident gateway restart"), CLI_NAME)}`));
			}
		}
		return;
	}
	if (!params.opts.json) {
		defaultRuntime.log("");
		if (params.result.mode === "npm" || params.result.mode === "pnpm") defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("trident doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("trident gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
		else defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("trident gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
	}
}
async function updateCommand(opts) {
	...
	if (opts.dryRun) {
		...
		else actions.push(`Run global package manager update with spec trident@${tag}`);
	...
	if (!opts.json) {
		defaultRuntime.log(theme.heading("Updating Trident..."));
		defaultRuntime.log("");
	}
	...
	if (result.status === "skipped") {
		...
		if (result.reason === "not-git-install") {
			defaultRuntime.log(theme.warn(`Skipped: this Trident install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("trident doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("trident gateway restart"), CLI_NAME)}\`.`));
			defaultRuntime.log(theme.muted(`Examples: \`${replaceCliName("npm i -g trident@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g trident@latest", CLI_NAME)}\``));
		}
		defaultRuntime.exit(0);
		return;
	}
...
async function updateWizardCommand(opts = {}) {
	if (!process.stdin.isTTY) {
		defaultRuntime.error("Update wizard requires a TTY. Use `trident update --channel <stable|beta|dev>` instead.");
		defaultRuntime.exit(1);
		return;
	}
	const timeoutMs = parseTimeoutMsOrExit(opts.timeout);
		...
		if (!await isGitCheckout(gitDir)) {
			if (await pathExists(gitDir)) {
				if (!await isEmptyDir(gitDir)) {
					defaultRuntime.error(`OPENCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENCLAW_GIT_DIR to an empty folder or an trident checkout.`);
					defaultRuntime.exit(1);
					return;
				}
			}
...
function registerUpdateCli(program) {
	const update = program.command("update").description("Update Trident and inspect update channel status").option("--json", "Output result as JSON", false).option("--no-restart", "Skip restarting the gateway service after a successful update").option("--dry-run", "Preview update actions without making changes", false).option("--channel <stable|beta|dev>", "Persist update channel (git + npm)").option("--tag <dist-tag|version>", "Override npm dist-tag or version for this update").option("--timeout <seconds>", "Timeout for each update step in seconds (default: 1200)").option("--yes", "Skip confirmation prompts (non-interactive)", false).addHelpText("after", () => {
		const fmtExamples = [
			["trident update", "Update a source checkout (git)"],
			["trident update --channel beta", "Switch to beta channel (git + npm)"],
			["trident update --channel dev", "Switch to dev channel (git + npm)"],
			["trident update --tag beta", "One-off update to a dist-tag or version"],
			["trident update --dry-run", "Preview actions without changing anything"],
			["trident update --no-restart", "Update without restarting the service"],
			["trident update --json", "Output result as JSON"],
			["trident update --yes", "Non-interactive (accept downgrade prompts)"],
			["trident update wizard", "Interactive update wizard"],
			["trident --update", "Shorthand for trident update"]
...
${theme.heading("Switch channels:")}
  ...
  - Run trident update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.trident.ai/cli/update")}`;
	}).action(async (opts) => {
		try {
			await updateCommand({
	...
	update.command("status").description("Show update channel and version status").option("--json", "Output result as JSON", false).option("--timeout <seconds>", "Timeout for update checks in seconds (default: 3)").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["trident update status", "Show channel + version status."],
		["trident update status --json", "JSON output."],
		["trident update status --timeout 10", "Custom timeout."]
	])}\n\n${theme.heading("Notes:")}\n${theme.muted("- Shows current update channel (stable/beta/dev) and source")}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.trident.ai/cli/update")}`).action(async (opts, command) => {
		try {
