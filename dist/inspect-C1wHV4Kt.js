import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName, r as GATEWAY_SERVICE_MARKER } from "./constants-CfAbrI_x.js";
import { r as resolveHomeDir } from "./runtime-parse-BCRWcre9.js";
import { i as execSchtasks } from "./service-B6O2Y9L1.js";
...
function renderGatewayServiceCleanupHints(env = process.env) {
	const profile = env.TRIDENT_PROFILE ?? env.OPENCLAW_PROFILE;
...
function hasGatewayServiceMarker(content) {
	const lower = content.toLowerCase();
	const markerKeys = ["trident_service_marker", "openclaw_service_marker"];
	const kindKeys = ["trident_service_kind", "openclaw_service_kind"];
...
function isOpenClawGatewayLaunchdService(label, contents) {
	...
	return label.startsWith("ai.trident.") || label.startsWith("ai.openclaw.");
}
function isOpenClawGatewaySystemdService(name, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!name.startsWith("trident-gateway") && !name.startsWith("openclaw-gateway")) return false;
	return contents.toLowerCase().includes("gateway");
}
function isOpenClawGatewayTaskName(name) {
	const normalized = name.trim().toLowerCase();
	if (!normalized) return false;
	return normalized === resolveGatewayWindowsTaskName().toLowerCase() || normalized.startsWith("trident gateway") || normalized.startsWith("openclaw gateway");
}
